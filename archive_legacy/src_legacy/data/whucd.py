...
Expected directory layout (auto-detected from config candidates):
...
OR flat (single-split, manual val split):

  root/
    A/  B/  label/

Folder name candidates are configurable via dataset config:
  image_a_dir_candidates: ["A", "imageA", "t1", "before"]
  image_b_dir_candidates: ["B", "imageB", "t2", "after"]
...
_A_CANDS     = ["A", "imageA", "t1", "before", "A_256", "T1"]
_B_CANDS     = ["B", "imageB", "t2", "after",  "B_256", "T2"]
_LABEL_CANDS = ["label", "labels", "mask", "OUT", "change_mask", "GT"]
...
class WHUCDDataset(Dataset):
    ...
    Returns dict with keys: image_a, image_b, label (=mask), mask, id, name.
    """
    ...
    def __init__(
        ...
        if split_dir is not None:
            ...
            # Flat layout: root/A  root/B  root/label — manual split
    ...
    def _check_dirs(self, parent: Path) -> None:
        for attr, name in [("a_dir", "A"), ("b_dir", "B"), ("lbl_dir", "label")]:
            if getattr(self, attr) is None:
                raise FileNotFoundError(
                    f"WHUCDDataset [{self.split}]: could not find {name} dir under {parent}. "
                    f"Check image_a/b/label_dir_candidates in dataset config."
                )
