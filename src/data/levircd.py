...
class LEVIRCDTileDataset(Dataset):
    """LEVIR-CD dataset that uses a pre-computed tile index.

    For train split the index is built with configurable stride (e.g. 128)
    so that each 1024×1024 image produces ~49 tiles.
    For val/test the stride defaults to tile_size (no overlap) to match the
    canonical evaluation tiles.

    All splits keep their A/B/label files untouched on disk; tiles are
    generated by cropping at index time.
    """
    ...
    def __init__(
        ...
        b_paths = [self.b_dir / n for n in names]
        m_paths = [self.lbl_dir / n for n in names]
        ...
        self.index: List[dict] = build_tile_index(
            ...
            image_paths_b       = b_paths,
            mask_paths          = m_paths,
            tile_size           = image_size,
            stride              = stride,
            min_change_pixels   = min_change_pixels,
            include_empty_ratio = include_empty_ratio if split == "train" else 1.0,
    ...
    # ── Statistics (for logging & sampler) ────────────────────────────────────
    def n_change(self) -> int:
        return sum(1 for e in self.index if e["has_change"])

    def n_no_change(self) -> int:
        return sum(1 for e in self.index if not e["has_change"])

    def change_ratio(self) -> float:
        return self.n_change() / max(1, len(self.index))

    def mean_changed_pixel_ratio(self) -> float:
        vals = [e["change_ratio"] for e in self.index if e["has_change"]]
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        img_a = np.array(
            Image.open(entry["image_a_path"]).convert("RGB")
        )[y : y + s, x : x + s]
        img_b = np.array(
            Image.open(entry["image_b_path"]).convert("RGB")
        )[y : y + s, x : x + s]
        lbl   = np.array(
            Image.open(entry["mask_path"]).convert("L")
        )[y : y + s, x : x + s]
        ...
        name = Path(entry["image_a_path"]).name
        return {
            "image_a":      ta,
            "image_b":      tb,
            "label":        tm,
