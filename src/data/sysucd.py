"""SYSU-CD multi-class change detection dataset (binary mode).

SYSU-CD contains 6 change types:
  impervious surfaces, building, low vegetation, trees, vehicles, boats.

For binary change detection all non-zero pixels are treated as change.
...
B) Flat with split text files (list of filenames):
   root/
     A/  B/  label/
     train.txt   val.txt   test.txt

C) Flat layout (manual 80/20 split):
   root/
     A/  B/  label/
...
_LABEL_CANDS = ["label", "mask", "OUT", "GT", "labels", "change_map", "cm"]
...
class SYSUCDDataset(Dataset):
    """SYSU-CD binary change detection dataset.
    ...
    def _check_dirs(self, parent: Path) -> None:
        for attr, label in [("a_dir", "A"), ("b_dir", "B"), ("lbl_dir", "label")]:
            if getattr(self, attr) is None:
                raise FileNotFoundError(
                    f"SYSUCDDataset [{self.split}]: could not find {label} dir under {parent}."
                )
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Binary: any non-zero pixel = change (SYSU is multi-class, merge all)
