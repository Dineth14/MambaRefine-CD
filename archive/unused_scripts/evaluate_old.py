...
def main() -> None:
    ...
    if second_eval:
        _CSV_KEYS = [
            "OA", "OA_change", "Fscd", "Fscd_binary_head", "Fscd_semantic_change",
            "mIoU", "SeK", "SeK_target", "SeK_transition",
            "binary_F1", "binary_IoU", "semantic_mIoU",
        ]
        _KEY_MAP = {key: (key,) for key in _CSV_KEYS}
    else:
        _CSV_KEYS = [
    ...
    if second_eval:
        second_json = out_dir / ("second_semantic_metrics.json" if results.get("second_eval_level") == "semantic_change" else "second_metrics.json")
        second_csv = out_dir / ("second_semantic_metrics.csv" if results.get("second_eval_level") == "semantic_change" else "second_metrics.csv")
        logger.info(f"Saved SECOND JSON → {second_json}")
        logger.info(f"Saved SECOND CSV  → {second_csv}")
    ...
    try:
        ...
        with torch.no_grad():
            logits = normalize_model_output(model(ia.to(device), ib.to(device)))["change_logits"]
